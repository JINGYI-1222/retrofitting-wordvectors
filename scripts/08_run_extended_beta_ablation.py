...
OUTPUT_CSV = RESULTS_DIR / "extended_beta_ablation_results.csv"
OUTPUT_LOG = RESULTS_DIR / "extended_beta_ablation_summary.txt"
...
def main() -> None:
    ...
    log("=== Extended Beta Ablation: GloVe + WordNet Synonym Retrofitting ===")
    log("This compares degree-normalized, constant, and symmetric-degree beta strategies.")
    log(f"Project root: {PROJECT_ROOT}")
    log(f"Embedding path: {EMBEDDING_PATH}")
    log(f"Max words: {MAX_WORDS}")
    log(f"Iterations: {NUM_ITERS}")
    log(f"Alpha: {ALPHA}")
    log(f"Beta strategies: {BETA_STRATEGIES}")
    log()
    ...
    for beta_strategy in BETA_STRATEGIES:
        log(f"=== Running retrofitting for beta_strategy={beta_strategy} ===")
        ...
        assert_all_vectors_finite(
            retrofitted, f"Retrofitted embeddings beta_strategy={beta_strategy}"
        )
        ...
        for dataset_name, pairs in datasets.items():
            ...
            if before["covered"] != after["covered"]:
                raise AssertionError(
                    f"Coverage mismatch for {dataset_name} at beta={beta_strategy}: "
            ...
            row = {
                "experiment_type": "extended_beta_ablation",
    ...
    log("=== Extended beta ablation completed successfully ===")
