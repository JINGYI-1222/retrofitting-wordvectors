...
ALPHAS = [0.1, 0.25, 0.5, 1.0, 2.0, 4.0, 10.0]

EMBEDDING_PATH = PROJECT_ROOT / "models/glove.6B.300d.txt"
RESULTS_DIR = PROJECT_ROOT / "results"
OUTPUT_CSV = RESULTS_DIR / "alpha_curve_results.csv"
OUTPUT_LOG = RESULTS_DIR / "alpha_curve_summary.txt"
OUTPUT_PLOT = RESULTS_DIR / "alpha_curve.png"
...
def plot_alpha_curve(rows: list[dict]) -> None:
    """Save an exploratory alpha sensitivity plot."""
    ...
    for dataset_name in datasets:
        ...
        dataset_rows = sorted(dataset_rows, key=lambda row: float(row["alpha"]))

        x_values = [float(row["alpha"]) for row in dataset_rows]
        y_values = [float(row["retrofitted_rho"]) for row in dataset_rows]

        plt.plot(x_values, y_values, marker="o", label=dataset_name)

    plt.xscale("log")
    plt.xlabel("Alpha (log scale)")
    plt.ylabel("Retrofitted Spearman rho")
    plt.title("Exploratory alpha sensitivity curve")
    plt.legend()
...
def main() -> None:
    ...
    log("=== Exploratory Alpha Curve: GloVe + WordNet Synonym Retrofitting ===")
    log("This is exploratory sensitivity analysis, not final hyperparameter optimization.")
    ...
    for alpha in ALPHAS:
        ...
        for dataset_name, pairs in datasets.items():
            ...
            row = {
                "experiment_type": "exploratory_alpha_curve",
    ...
    log("=== Exploratory alpha curve completed successfully ===")
