"""Run full-data WordNet graph and beta-strategy ablations.

This script compares WordNet synonym-only and expanded WordNet graphs across
the beta weighting strategies supported by src.retrofit.retrofit_vectors.
"""
...
DEFAULT_OUTPUT_PATH = (
    PROJECT_ROOT / "results/wordnet_variant_beta_ablation_full.csv"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare WordNet graph variants and beta strategies for "
            "retrofitting GloVe 6B 300d embeddings."
        )
    )
    parser.add_argument(
        "--embedding-path",
...
def main() -> None:
    ...
    print("=== WordNet Variant Beta Ablation ===")
    ...
    for graph_variant, graph_options in GRAPH_VARIANTS.items():
        print(f"Building {graph_variant} graph...")
        graph = build_wordnet_graph(embedding_vocab, **graph_options)
        graph = filter_graph_by_vocab(graph, embedding_vocab)
        graph_nodes = len(graph)
        graph_edges = count_undirected_edges(graph)
        print(f"{graph_variant}: nodes={graph_nodes}, edges={graph_edges}")
        print()
    ...
    print("Completed WordNet variant beta ablation.")
